...
(() => {
// test
const { execSync, spawn } = __nccwpck_require__(81);
...
(async () => {
  ...
  const checkLastCommitOnly =
    process.env["INPUT_CHECK-LAST-COMMIT-ONLY"] || "false";

  let messages = [];
  if (checkLastCommitOnly === "true") {
    console.log("Only checking the last commit...");
    const commit =
      event.commits && event.commits.lengths > 0
        ? event.commits[event.commits.length - 1]
        : null;
    messages = commit ? [commit.message + "\n" + commit.body] : [];
  } else {
    messages = event.commits
      ? event.commits.map((commit) => commit.message + "\n" + commit.body)
      : [];
  }

  const commitMessage =
    process.env["INPUT_COMMIT-MESSAGE"] || "ci: version bump to {{version}}";
  ...
  if (bumpPolicy === "all") {
    isVersionBump =
      messages.find((message) => commitMessageRegex.test(message)) !==
      undefined;
  ...
  const patchWords = process.env["INPUT_PATCH-WORDING"]
    ...
    : null;
  const preReleaseWords = process.env["INPUT_RC-WORDING"]
    ? process.env["INPUT_RC-WORDING"].split(",")
    : null;
  ...
  if (
    ...
    preReleaseWords &&
    !messages.some((message) =>
      preReleaseWords.some((word) => message.includes(word))
    ) &&
    !versionType
  ) {
    version = null;
  ...
  try {
    ...
    newVersion = `${tagPrefix}${newVersion}${tagSuffix}`;
    console.log(
      `newVersion after merging tagPrefix+newVersion+tagSuffix: ${newVersion}`
    );
